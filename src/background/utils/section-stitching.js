...
export default async function stitchSections({
  rect,
  maxSectionHeight,
  captureScreenshotFunc,
}) {
  ...
  if (numberOfSections === 1) {
    ...
    const png = await makePngFromBuffer(
  ...
  for (const i of range(0, numberOfSections)) {
    ...
    const screenshotResult = await captureScreenshotFunc({ rect: clip })
    const section = await makePngFromBuffer(
      Buffer.from(screenshotResult.data, 'base64')
    )
